...
export function Navbar() {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(
      navBarRef.current,
      { opacity: 0, scale: 0.9, translateY: "-20px" },
      { opacity: 1, scale: 1, translateY: "0px", duration: 0.8, ease: "power2.out", delay: 0.5 }
    );
  ...
  return (
    <>
      <div className="fixed top-0 left-0 w-full h-[80px] flex justify-center items-center px-6 lg:px-[10vw] z-50">
        <div
          ref={navBarRef}
          className={`w-full h-[56px] border flex justify-between items-center px-6 pr-3 rounded-full opacity-0 transition-colors duration-300 ${
            scrolled
              ? "border-black/8 bg-white/95 shadow-sm"
