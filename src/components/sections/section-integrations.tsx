...
const DELEGATION_KEYS = ["scoped", "chain", "revoke"] as const;
...
export function SectionIntegrations() {
	...
	// Auto-rotate delegation features
	useEffect(() => {
	...
	useEffect(() => {
		if (!emblaApi) return;
		const onSelect = () => setDelegationIndex(emblaApi.selectedScrollSnap());
	...
	return (
		<section ref={sectionRef} id="features" className="relative px-6 lg:px-[10vw]">
				...
				<div ref={gridRef} className="bento-grid" style={{ perspective: "1000px" }}>
					...
					<div className="bento-item md:row-span-2 flex flex-col">
						...
						<div className="relative z-10">
							<h3 className="headline-sm text-foreground mb-2">{t("agentIdentity.title")}</h3>
							<p className="body-md">
								{t("agentIdentity.description")}
							</p>
						</div>
						<div className="relative z-10 flex gap-4 mt-auto pt-6">
							<div className="rounded-2xl border border-accent/15 bg-accent/5 p-4 flex-1 text-center">
								<div className="text-lg font-bold text-foreground">{t("agentIdentity.create")}</div>
								<div className="text-xs text-foreground-secondary mt-1">{t("agentIdentity.createSub")}</div>
							</div>
							<div className="rounded-2xl border border-accent/15 bg-accent/5 p-4 flex-1 text-center">
								<div className="text-lg font-bold text-foreground">{t("agentIdentity.resolve")}</div>
								<div className="text-xs text-foreground-secondary mt-1">{t("agentIdentity.resolveSub")}</div>
							</div>
						</div>
						<div className="ambient-glow" style={{ width: "200px", height: "200px", right: "-40px", top: "-40px" }} />
					</div>

					{/* Small: Delegation carousel */}
					<div className="bento-item flex flex-col justify-between min-h-[180px]">
						<div className="grid-overlay" />
						<div className="relative z-10">
							<h3 className="text-sm font-semibold text-foreground mb-3">{t("delegation.title")}</h3>
							<div ref={emblaRef} className="embla overflow-hidden">
								<div className="embla__container">
									{DELEGATION_KEYS.map((key) => (
										<div key={key} className="embla__slide">
											<div className="font-bold text-foreground mb-1">{t(`delegation.${key}.name`)}</div>
											<p className="text-xs text-foreground-secondary leading-relaxed">{t(`delegation.${key}.description`)}</p>
										</div>
									))}
								</div>
							</div>
							...
							<div className="flex gap-1.5 mt-3">
								{DELEGATION_KEYS.map((key, i) => (
									<div
										key={key}
										className={`w-1.5 h-1.5 rounded-full transition-colors duration-300 ${
											i === delegationIndex ? "bg-accent" : "bg-black/10"
										}`}
									/>
								))}
							</div>
